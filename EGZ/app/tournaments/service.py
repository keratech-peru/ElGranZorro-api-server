...
class Tournaments_(CRUD):
    ...
    def list_all_is_enrollend_user(db: Session, appuser_id: int) -> List[Tournaments]:
        ...
        for tournament in tournaments:
            ...
            if tournament_["is_enrolled_user"]:
                ...
                tournament_["active_for_user"] = int(tournament_["level"]) <= int(db.query(AppUsers.level).filter(AppUsers.id == appuser_id).first()[0])
                tournaments_.append(tournament_)
        return tournaments_
    ...
    def get_data_group_table(db: Session, tournament_id: int , user_id: int):
        ...
        for group_stage in groups_stage:
            ...
            group_stage_["team_name"] = appuser.team_name if appuser else None
...
class FootballGames_(CRUD):
    ...
    def list_search_codigo(db: Session, codigo: str, date: str) -> List[FootballGames]:
        ...
        for footballgame in footballgames:
            ...
            if not is_over(tournament.start_date):
                ...
                footballgame_["is_past_date"] = is_past(footballgame.date) if footballgame.date else None
                footballgame_["is_past_hour"] = is_past(footballgame.date,footballgame.hour) if footballgame.date else None
                ...
                search_date = (date in footballgame.date) if footballgame.date else True 
                if codigo in footballgame_["codigo"] and search_date:
    ...
    def create_keys_stage(id: int , codigo:str, start_date:str, stage: str, db: Session):
        list_footballgames = []
        for i in range(1,4):
            date = datetime.strptime(start_date, '%d/%m/%y') + timedelta(days = FOOTBALLGAMES_BY_STAGE[stage]["days_after_starting"])
            obj_temp = schemas.FootballGames(
...
class Confrontations_(CRUD):
    ...
    def create_keys_stage(tournament_id: int, db: Session, cod_tournament: str, stage: str):
        list_confrontations = []
        for _ in range(FOOTBALLGAMES_BY_STAGE[stage]["number_competencies"]):
